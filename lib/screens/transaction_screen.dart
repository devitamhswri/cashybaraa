...
class _TransactionScreenState extends State<TransactionScreen> {
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      Provider.of<TransactionProvider>(context, listen: false)
          .loadMonth(_focusedDay.year, _focusedDay.month); // ← fix
    });
  }
        ...
        return Scaffold(
                    ...
                    children: [
                          ...
                          children: [
                            GestureDetector(
                              onTap: () {
                                ...
                                provider.loadMonth(_focusedDay.year, _focusedDay.month); // ← fix
                              },
                              ...
                              .map((d) => Expanded(
                                    child: Center(
                                      child: Text(d,
                                          style: TextStyle(
                                              fontSize: 11,
                                              color: Colors.white.withValues(alpha: 0.6), // ← fix
                      ...
                      Container(
                        ...
                        decoration: BoxDecoration(
                          color: Colors.white.withValues(alpha: 0.12), // ← fix
                              ...
                              children: [
                                Text(_labelHari(_selectedDay),
                                    style: const TextStyle(
                                        fontSize: 13, fontWeight: FontWeight.w700, color: Color(0xFF2D2218))),
                                Text(
                                  saldoHari >= 0
          ...
          floatingActionButton: FloatingActionButton(
            ...
            onPressed: () async {
              ...
              // Reload setelah kembali dari add screen
              if (mounted) {
                ...
                child: Container(
                  ...
                  decoration: BoxDecoration(
                    ...
                    border: isToday && !isSelected
                        ? Border.all(color: Colors.white.withValues(alpha: 0.4), width: 1) // ← fix
                        : null,
                  ),
                  child: Column(
                    mainAxisAlignment: MainAxisAlignment.center,
  ...
  Widget _buildTransaksiCard(dynamic t) {
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        color: Colors.white,
        borderRadius: BorderRadius.circular(16),
        border: Border.all(color: Colors.black.withValues(alpha: 0.05), width: 0.5), // ← fix
      ),
      child: Row(
        children: [
          Container(
  ...
  Widget _summaryItem(String label, double amount, Color color) {
    return Expanded(
      child: Column(
        children: [
          Text(label,
              style: TextStyle(fontSize: 9, color: Colors.white.withValues(alpha: 0.6), letterSpacing: 0.5)), // ← fix
          const SizedBox(height: 4),
          Text(_formatRp(amount.abs().toInt()),
  ...
  Widget _divider() => Container(
        width: 1, height: 30,
        color: Colors.white.withValues(alpha: 0.15), // ← fix
      );

  bool _isSameDay(DateTime a, DateTime b) =>
      a.year == b.year && a.month == b.month && a.day == b.day;

  String _namabulan(int m) => ['', 'Januari', 'Februari', 'Maret', 'April', 'Mei', 'Juni',
